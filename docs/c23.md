...
```c
bool x = true;  // No need for <stdbool.h>
```
**Difficulty:** Easy - Add as keywords, define `bool` as `_Bool`.

#### `nullptr` keyword
```c
int *p = nullptr;  // Type-safe null pointer
```
**Difficulty:** Easy - New keyword with type `nullptr_t`.
...
#endif
```
**Difficulty:** Easy - Add to preprocessor directive handling.
...
```
**Difficulty:** Easy - Emit warning message during preprocessing.
...
3. `#warning` directive
4. `#elifdef` / `#elifndef`
5. `[[maybe_unused]]` attribute
6. `[[nodiscard]]` attribute
7. `[[deprecated]]` attribute
8. `true`/`false` keywords
9. `nullptr` keyword
10. `typeof_unqual`
11. Labels before `}`
